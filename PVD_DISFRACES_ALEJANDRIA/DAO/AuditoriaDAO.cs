...
{
    /// <summary>
    /// Clase de acceso a datos para auditoría de productos
    /// </summary>
    internal class AuditoriaDAO
    {
        private readonly Conexion conexion;

        // Constructor que crea la instancia de conexión
        public AuditoriaDAO()
        {
        ...
        public List<AuditoriaProducto> ListarTodos()
        {
            ...
            try
            {
                ...
                using var cmd = new MySqlCommand(query, conn); // Crear el comando SQL
                using var reader = cmd.ExecuteReader(); // Ejecuta el comando y ejecuta el lector

                while (reader.Read()) // Lee cada registro
                {
                    var auditoria = new AuditoriaProducto // Crea una instancia de AuditoriaProducto
                    {
